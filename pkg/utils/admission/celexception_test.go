...
func Test_UnmarshalCELPolicyException(t *testing.T) {
	testCases := []struct {
		name      string
		raw       []byte
		expectErr bool
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			result, err := UnmarshalCELPolicyException(test.raw)
			if test.expectErr {
				if err == nil {
...
func TestGetCELPolicyExceptions(t *testing.T) {
	type args struct {
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			if err != nil {
				...
				expectedP2, err := UnmarshalCELPolicyException(test.args.request.OldObject.Raw)
				if err != nil {
					t.Errorf("Unexpected error: %v", err)
				}
				if !reflect.DeepEqual(expectedP1, p1) || !reflect.DeepEqual(expectedP2, p2) {
